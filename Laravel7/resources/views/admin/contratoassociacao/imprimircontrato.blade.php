...
<br />
Nome do (a) Aluno (a): <b>{{ $contratoassociacao->nome }}</b>
<br />
DATA DE NASCIMENTO: <b>{{ date('d/m/Y', strtotime($contratoassociacao->nascimento)) }}</b>, IDADE <b>{{ date('Y')-date('Y', strtotime($contratoassociacao->nascimento)) }} ANOS</b>, CPF: <b>{{ $contratoassociacao->cpf }}</b>
<br />
Cartão do SUS nº <b>{{ $contratoassociacao->cartaosus }}</b>
<br />
Possui Plano de Saúde? <b>{{ $contratoassociacao->planodesaude == 'S' ? 'Sim. '.$contratoassociacao->nomeplanodesaude : 'Não.' }}</b>
<br />
Possui alguma alergia a alimentos? <b>{{ $contratoassociacao->alergia == 'S' ? 'Sim. '.$contratoassociacao->nomealergia : 'Não.' }}</b>
<br />
Possui alguma alergia a Medicamentos? <b>{{ $contratoassociacao->alergiaalimento == 'S' ? 'Sim. '.$contratoassociacao->nomealergiaalimento : 'Não.' }}</b>
<br />
Faz Uso de Algum Medicamento? <b>{{ $contratoassociacao->medicamentos == 'S' ? 'Sim. '.$contratoassociacao->nomemedicamentos : 'Não.' }}</b>
<br />
Possui Alguma Necessidade Especial? <b>{{ $contratoassociacao->necessidadesfisicas == 'S' ? 'Sim. '.$contratoassociacao->nomenecessidadesfisicas : 'Não.' }}</b>
<br />
Autorizo o Uso de Imagem do Aluno Matriculado Fotos e Vídeos nas Redes Sociais e para Propaganda do Espaço pedagógico? <b>{{ $contratoassociacao->autorizaredessociais == 'S' ? 'Sim.' : 'Não.' }}</b>
<br />
Autoriza alguém buscar o aluno? <b>{{ $contratoassociacao->autorizabuscaraluno == 'S' ? 'Sim. '.$contratoassociacao->autorizabuscaralunonome : 'Não.' }}</b>
<br />
Modalidade CONTRATADA: <b>{{ $contratoassociacao->modalidade }}</b>
<br />
Responsável Financeiro: <b>{{ $contratoassociacao->nomeresponsavelfinanceiro }}</b> - CPF: {{ $contratoassociacao->cpfresponsavelfinanceiro }} - Telefone: {{ $contratoassociacao->numerotelefoneresponsavelfinanceiro }}
<br />
1) Nome da Mãe ou Responsável: {{ $contratoassociacao->nomemae.' - CPF: '.$contratoassociacao->cpfmae.' - Endereço: '.$contratoassociacao->enderecomae.' - Bairro: '.$contratoassociacao->bairromae.' - Cidade: '.$contratoassociacao->cidademae.'-'.$contratoassociacao->estadomae.' - Telefone: '.$contratoassociacao->numerotelefonemae.' '.$contratoassociacao->telefonetrabalhomae.' - e-mail: '.$contratoassociacao->emailmae }}
<br />
2) Nome do Pai ou Responsável: {{ $contratoassociacao->nomepai.' - CPF: '.$contratoassociacao->cpfpai.' - Endereço: '.$contratoassociacao->enderecopai.' - Bairro: '.$contratoassociacao->bairropai.' - Cidade: '.$contratoassociacao->cidadepai.'-'.$contratoassociacao->estadopai.' - Telefone: '.$contratoassociacao->numerotelefonepai.' '.$contratoassociacao->telefonetrabalhopai.' - e-mail: '.$contratoassociacao->emailpai }}
<br />
Pelo presente contrato particular, que fazem entre si, {{ $contratoassociacao->razaosocial }}, pessoa jurídica de direito privado, com sede na {{ $contratoassociacao->enderecorazaosocial }}, Inscrita no CNPJ sob o nº {{ $contratoassociacao->cnpj }}, com Inscrição Municipal nº {{ $contratoassociacao->inscricaomunicipal }}, daqui por diante denominada simplesmente CONTRATADA, e do outro lado às partes acima qualificadas, doravante denominadas simplesmente CONTRATANTES, têm, entre si, como justas e contratadas as seguintes cláusulas e condições constantes no presente Contrato de Prestação de Serviços Educacionais por adesão:
<br />
1. OBJETO - O objeto do presente contrato é a prestação de serviços educacionais pela CONTRATADA, correspondente a educação infantil, período e turno requerido pelos CONTRATANTES, para benefício do(a) menor que representam, para o ano letivo de {{ $contratoassociacao->anoletivo }}.
<br />
...
<br />
6.7. Nesse ato de Matrícula também autorizo {{ $contratoassociacao->autorizabuscaralunonome }}, a retirar meu filho na impossibilidade dos Pais que será comunicado em tempo hábil para o responsável do Espaço Pedagógico organizar a saída.
<br />
...
<br />
10. HORAS EXTRAS – As crianças de horário parcial, poderão, em caso de necessidade dos pais, permanecer na associacao. O tempo excedente ao contratado, para a entrada ou para saída será anotado e cobrado no boleto do mês subsequente. Não haverá compensação de horário, ou seja, a criança que chegar mais tarde não terá o direito de permanecer além do horário contratado sem pagar hora extra. As crianças que permanecerem após às 19 (dezenove) horas pagarão uma taxa a cada quarto de hora que se iniciar, conforme ANEXO 1.
<br />
11. MATERIAL ESCOLAR - A CONTRATADA se responsabiliza pela aquisição do material associacaor de uso comum.
<br />
11.1. Os CONTRATANTES são responsáveis pela aquisição de todo o material associacaor individual e produtos de Higiene Pessoal individual, conforme exigência do CONTRATADA.
<br />
...
<b>Anexo 1 </b>
<br />
Anuidades - Ano {{ $contratoassociacao->anoletivo }}
<br /><br />
<b>Valores</b>
<br />
{{ $contratoassociacao->valorintegral }}
<br />
{{ $contratoassociacao->valorparcial }}
<br /><br />
<b>Horas extras</b>
<br />
1 Hora extra = R$ {{ $contratoassociacao->valorhoraextra }}
<br />
1 Refeição extra = R$ {{ $contratoassociacao->valorrefeicaoextra }}
<br />
Após às 19 horas, será cobrada uma taxa de {{ $contratoassociacao->valor19horas }}
<br /><br />
<b>Horário de Funcionamento</b>
<br />
De Segunda à Sexta 
<br />
Integral: {{ $contratoassociacao->horariointegral }}
<br />
Parcial: {{ $contratoassociacao->horarioparcial }}
<br /><br />
{{ $contratoassociacao->anexotexto }}
<br />
As partes elegem o Foro da Comarca de {{ $contratoassociacao->cidadeforo }}, como o único competente para dirimir eventuais questões resultantes da interpretação ou execução do presente contrato.
<br /><br />
Nova Andradina – MS, {{ $contratoassociacao->datadocontrato }}.
<br /><br /><br /><br />
<div class="col-12 row">
    <div class="col-6">
        <hr />
        {{ $contratoassociacao->nomeresponsavelfinanceiro }}
        <br />
        CPF: {{ $contratoassociacao->cpfresponsavelfinanceiro }}
        <br />
    ...
    <div class="col-6">
        <hr />
        {{ $contratoassociacao->razaosocial }}
    </div>
</div>
<br /><br />
