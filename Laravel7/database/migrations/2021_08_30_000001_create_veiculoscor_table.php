...
class CreateVeiculoscorTable extends Migration {
     ...
     */
    public function up() {
        Schema::create('veiculoscor', function (Blueprint $table) {
            $table->increments('id');
            $table->index('id');
            $table->unsignedInteger('empresa_id')->comment('ID da empresa multi-empresa');
            $table->foreign('empresa_id')->references('id')->on('empresas')->onDelete('cascade');
            
            $table->string('cor')->comment('Nome da cor');
    ...
    public function down() {
        Schema::dropIfExists('veiculoscor');
    }
