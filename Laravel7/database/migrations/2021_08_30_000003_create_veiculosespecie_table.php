...
class CreateVeiculosespecieTable extends Migration {
     ...
     */
    public function up() {
        Schema::create('veiculosespecie', function (Blueprint $table) {
            $table->increments('id');
            $table->index('id');
            $table->unsignedInteger('empresa_id')->comment('ID da empresa multi-empresa');
            $table->foreign('empresa_id')->references('id')->on('empresas')->onDelete('cascade');
            
            $table->string('especie')->comment('Nome da espécie'); 
    ...
    public function down() {
        Schema::dropIfExists('veiculosespecie');
    }
