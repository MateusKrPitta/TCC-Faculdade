...
class CreateVeiculoscombustivelTable extends Migration {
     ...
     */
    public function up() {
        Schema::create('veiculoscombustivel', function (Blueprint $table) {
            $table->increments('id');
            $table->index('id');
            $table->unsignedInteger('empresa_id')->comment('ID da empresa multi-empresa');
            $table->foreign('empresa_id')->references('id')->on('empresas')->onDelete('cascade');
            
            $table->string('combustivel')->comment('Nome do tipo de combustível'); 
    ...
    public function down() {
        Schema::dropIfExists('veiculoscombustivel');
    }
