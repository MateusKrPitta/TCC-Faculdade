...
class CreateVeiculoslinhaTable extends Migration {
     ...
     */
    public function up() {
        Schema::create('veiculoslinha', function (Blueprint $table) {
            $table->increments('id');
            $table->index('id');
            $table->unsignedInteger('empresa_id')->comment('ID da empresa multi-empresa');
            $table->foreign('empresa_id')->references('id')->on('empresas')->onDelete('cascade');
            
            $table->string('linha')->comment('Nome da marca');
    ...
    public function down() {
        Schema::dropIfExists('veiculoslinha');
    }
