...
class CreateVeiculosrodadoTable extends Migration {
     ...
     */
    public function up() {
        Schema::create('veiculosrodado', function (Blueprint $table) {
            $table->increments('id');
            $table->index('id');
            $table->unsignedInteger('empresa_id')->comment('ID da empresa multi-empresa');
            $table->foreign('empresa_id')->references('id')->on('empresas')->onDelete('cascade');
            
            $table->string('rodado')->comment('Nome do tipo de rodado');
    ...
    public function down() {
        Schema::dropIfExists('veiculosrodado');
    }
