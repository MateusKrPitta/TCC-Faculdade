...
class CreateProcedimentosTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('procedimentos', function (Blueprint $table) {
            ...
            $table->unsignedInteger('especialidade_id')->comment('ID da especialidade');
            $table->foreign('especialidade_id')->references('id')->on('especialidades')->onDelete('cascade');

            $table->String('nome')->comment('Nome da especialidade');
            $table->decimal('tempo',10,2)->comment('Tempo em Horas');
            $table->decimal('valor')->comment('Valor do Procedimento');
